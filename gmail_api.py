...
def get_email_messages(service, user_id='me', label_ids=None, folder_name='INBOX', max_results=5):
    ...
    if folder_name:
        labels_results = service.users().labels().list(userId=user_id).execute()
...
def get_email_message_details(service, msg_id):
    ...
    reciepients = next((header['value'] for header in headers if header['name'] == 'To'), 'No reciepients')
    snippet = message.get('snippet', 'No snipper')
    has_attachement = any(part.get('filename') for part in payload.get('parts', []) if part.get('filename'))
    ...
    return {
        ...
        'body' : body,
        'snippet' : snippet,
        'has_attachments' : has_attachement,
        'date' : date,
        'star' : star,
        'label' : label,     
    }

# SPACE TO MATCH THE VIDEO:
# GMAIL API CRASH COURSE: AUTOMATE GMAIL EMAIL TASKS IN PYTHON | BEGINNER TUTORIAL
# MADE BY JIE JENN

# f"{subject} & {sender} & {reciepients} & {body} & {snippet} & {has_attachement} & {date} & {star} & {label} $"
